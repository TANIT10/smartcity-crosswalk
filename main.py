...
SAMPLE_VIDEO_FILE = os.path.join(
    BASE_DIR,
    "sample.mp4"
)
...
def switch_camera():
    ...
    if DEPLOY_MODE:
        return jsonify({
            "success": False,
            "message": "배포 데모에서는 CCTV 전환을 사용하지 않습니다."
        }), 409
...
if DEPLOY_MODE:
    print(
        "[배포] YouTube 대신 sample.mp4를 사용합니다."
    )

    if not os.path.exists(SAMPLE_VIDEO_FILE):
        print(
            "[오류] sample.mp4 파일이 없습니다."
        )
        raise SystemExit
...
if DEPLOY_MODE:
    print()
    print(
        "[배포] 저장된 camera1 ROI를 sample.mp4에 적용합니다."
    )
